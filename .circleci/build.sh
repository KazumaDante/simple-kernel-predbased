...
git clone --depth=1 https://github.com/KazuDante89/simple-kernel-predbased -b EAS3 kernel
cd kernel
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 [EAS]-Simple-Kernel-R10.2_v0.0-${TANGGAL}.zip *
    cd ..
}
sendinfo
compile
